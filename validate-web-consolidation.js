...
for(const text of ['Inteligência Comercial','Movvant Enterprise','id="scope"','id="branch"','id="user"','Ranking de lojas','Ranking de vendedores','Carteira de clientes','Produtos e mix','Clientes em risco','Oportunidades prioritárias','Eficiência de campo','Qualidade dos dados','enterprise-intelligence.js','enterprise-rankings.js']){if(!html.includes(text))throw new Error(`Missing dashboard item: ${text}`)}
...
const ranks=fs.readFileSync('dist/enterprise-rankings.js','utf8');
for(const text of ['commercial_dimension_monthly','branchMap','customerRanking','productRanking','trend_pct','dimension_type']){if(!ranks.includes(text))throw new Error(`Missing dimensional ranking capability: ${text}`)}
...
console.log('Movvant Web Consolidation V1: Enterprise + rankings loja/vendedor/cliente/produto + risco/oportunidade OK');