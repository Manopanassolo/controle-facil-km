const fs=require('fs');
const required=['dist/index.html','dist/commercial-intelligence.html','dist/commercial-intelligence-core.js','dist/authenticated-intelligence.js','dist/navigation-manifest.js','dist/access-scope.js','dist/historical-data-contract.js','dist/contract-profile.js','dist/product-catalog-contract.js','dist/sales-import-contract.js','dist/sales-fact-model.js','dist/shell-bridge.js'];
for(const f of required){if(!fs.existsSync(f))throw new Error(`Missing ${f}`)}
const main=fs.readFileSync('dist/index.html','utf8');
for(const text of ['MOVVANT_WEB_CONSOLIDATION_V1','/navigation-manifest.js','/access-scope.js','/historical-data-contract.js','/contract-profile.js','/product-catalog-contract.js','/sales-import-contract.js','/sales-fact-model.js','/shell-bridge.js']){if(!main.includes(text))throw new Error(`Missing shell integration: ${text}`)}
...
const profile=fs.readFileSync('dist/contract-profile.js','utf8');
for(const text of ['MOVVANT_CONTRACT_PROFILE','allowCustomDimensions','metrics','benchmark','mobile','web']){if(!profile.includes(text))throw new Error(`Missing contract profile capability: ${text}`)}
const catalog=fs.readFileSync('dist/product-catalog-contract.js','utf8');
for(const text of ['MOVVANT_PRODUCT_CATALOG','productCode','dedupeMaster','facts','branchId','snapshotAt','latestSaleProduct','resolveMaster','latest_sale']){if(!catalog.includes(text))throw new Error(`Missing product catalog capability: ${text}`)}
const salesImport=fs.readFileSync('dist/sales-import-contract.js','utf8');
for(const text of ['MOVVANT_SALES_IMPORT','sourceFileHash','batchRowId','businessFingerprint','productName','operation']){if(!salesImport.includes(text))throw new Error(`Missing sales import capability: ${text}`)}
...
console.log('Movvant Web Consolidation V1: authenticated scope + mobility + configurable products + historical sales resolution OK');
