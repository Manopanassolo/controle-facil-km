...
try{
  ...
  await page.evaluate(()=>globalThis.mvNavigationV164?.openMenu?.());await page.waitForTimeout(100);result.menuOpen=await state();
  await page.locator('#mvMenu164 [data-page164="agenda"]').click();await page.waitForTimeout(180);result.agenda=await state();
  await page.waitForTimeout(3000);result.agendaStable=await page.evaluate(()=>({page:document.body.dataset.mvPage,visible:!document.getElementById('p-agenda')?.classList.contains('hide'),display:getComputedStyle(document.getElementById('p-agenda')).display,authority:document.body.dataset.mvNavAuthority||''}));
  await navTo('inicio');result.home=await page.evaluate(()=>({page:document.body.dataset.mvPage,visible:!document.getElementById('p-inicio')?.classList.contains('hide')}));
  ...
  result.pass=Object.values(result.shell).every(Boolean)&&result.initial.authority==='164.2'&&result.history.page==='historico'&&result.history.stored==='historico'&&result.history.visible&&result.menuOpen.menuOpen&&result.agenda.dataset==='agenda'&&!result.agenda.menuOpen&&result.agendaStable.page==='agenda'&&result.agendaStable.visible&&result.agendaStable.display!=='none'&&result.agendaStable.authority==='164.2'&&result.home.page==='inicio'&&result.home.visible&&result.reload.page==='agenda'&&result.reload.stored==='agenda'&&result.reload.visible&&result.reload.authority==='164.2';
