...
const js=`
// v163.13: unified safe autocomplete; preserve tapped choice until gesture completes.
(function(){
  const routeIds=new Set(['origem','destino','preTripStopNameV127']);
  let active=null,timer=0,items=[],selectedIndex=-1,boundTarget=null,closeTimer=0;
  ...
  function close(){clearTimeout(closeTimer);const p=portal();p.classList.add('hide');p.innerHTML='';items=[];selectedIndex=-1;boundTarget=null}
  ...
  function render(arr,input){clearTimeout(closeTimer);items=Array.isArray(arr)?arr:[];selectedIndex=-1;boundTarget=input;if(!items.length){close();return}const p=portal();p.dataset.targetId=input.id;p.innerHTML=items.map((x,i)=>'<button type="button" data-mv-choice63="'+i+'"><b>'+esc(x.mainText||x.text)+'</b><span>'+esc(x.secondaryText||'')+'</span><em>Toque para selecionar</em></button>').join('')+'<div class="mv-route-google-v16263">Resultados fornecidos pelo Google</div>';position(input);p.classList.remove('hide')}
  ...
  function confirmSelection(index,b){const x=items[index];const target=boundTarget;if(!isField(target))return;const value=choiceText(b,x);if(!value)return;clearTimeout(timer);target.value=value;target.dataset.placeId=x?.placeId||'';active=target;selectedIndex=-1;const p=portal();p.style.pointerEvents='none';closeTimer=setTimeout(()=>{p.style.removeProperty('pointer-events');close()},180)}
  function handleChoice(b,e){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation?.();const idx=Number(b.dataset.mvChoice63);if(!Number.isInteger(idx))return;const p=portal();if(selectedIndex===idx||b.classList.contains('mv-selected-v16263')){confirmSelection(idx,b);return}clearTimeout(timer);selectedIndex=idx;p.querySelectorAll('button[data-mv-choice63]').forEach((btn,i)=>{const sel=i===idx;btn.classList.toggle('mv-selected-v16263',sel);const em=btn.querySelector('em');if(em)em.textContent=sel?'Selecionado — toque novamente para confirmar':'Toque para selecionar'})}
...
`;
if(!s.includes('carga();'))throw new Error('v163.13 startup anchor not found');s=s.replace('carga();',js+'\ncarga();');
const css=`
/* v163.13 safe Android autocomplete */
...
`;
if(!s.includes('</style>'))throw new Error('v163.13 css anchor not found');s=s.replace('</style>',css+'\n</style>');fs.writeFileSync('dist/index.html',s);console.log('Movvant v163.13: gesture-complete two-tap Android autocomplete installed');