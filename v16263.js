...
const js=`
// v163.14: unified safe autocomplete; hide instantly, clean tapped choice after gesture completes.
(function(){
  ...
  function resetPortal(){clearTimeout(closeTimer);const p=portal();p.classList.add('hide');p.style.removeProperty('pointer-events');p.innerHTML='';items=[];selectedIndex=-1;boundTarget=null}
  ...
  function confirmSelection(index,b){const x=items[index];const target=boundTarget;if(!isField(target))return;const value=choiceText(b,x);if(!value)return;clearTimeout(timer);target.value=value;target.dataset.placeId=x?.placeId||'';active=target;selectedIndex=-1;const p=portal();p.classList.add('hide');p.style.pointerEvents='none';closeTimer=setTimeout(()=>{p.style.removeProperty('pointer-events');p.innerHTML='';items=[];boundTarget=null},180)}
...
`;
if(!s.includes('carga();'))throw new Error('v163.14 startup anchor not found');s=s.replace('carga();',js+'\ncarga();');
const css=`
/* v163.14 safe Android autocomplete */
...
`;
if(!s.includes('</style>'))throw new Error('v163.14 css anchor not found');s=s.replace('</style>',css+'\n</style>');fs.writeFileSync('dist/index.html',s);console.log('Movvant v163.14: instant-hide gesture-safe autocomplete installed');