...
const js=`
// v163.12: unified safe autocomplete with deferred post-confirmation change event.
(function(){
  ...
  function confirmSelection(index,b){const x=items[index];const target=boundTarget;if(!isField(target))return;const value=choiceText(b,x);if(!value)return;clearTimeout(timer);target.value=value;target.dataset.placeId=x?.placeId||'';close();active=target;setTimeout(()=>{try{target.dispatchEvent(new Event('change',{bubbles:true}))}catch(_){}},0)}
...
`;
if(!s.includes('carga();'))throw new Error('v163.12 startup anchor not found');s=s.replace('carga();',js+'\ncarga();');
const css=`
/* v163.12 safe Android autocomplete */
...
`;
if(!s.includes('</style>'))throw new Error('v163.12 css anchor not found');s=s.replace('</style>',css+'\n</style>');fs.writeFileSync('dist/index.html',s);console.log('Movvant v163.12: deferred autocomplete confirmation installed');