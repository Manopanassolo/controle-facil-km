...
const js=`
// v163.15: unified autocomplete; second tap commits on pointerdown, avoiding Android gesture hang.
(function(){
  const routeIds=new Set(['origem','destino','preTripStopNameV127']);
  let active=null,timer=0,items=[],selectedIndex=-1,boundTarget=null,closeTimer=0,commitGesture=false;
  ...
  function resetPortal(){clearTimeout(closeTimer);const p=portal();p.classList.add('hide');p.style.removeProperty('pointer-events');p.innerHTML='';items=[];selectedIndex=-1;boundTarget=null;commitGesture=false}
  ...
  function commit(index,b){const x=items[index],target=boundTarget;if(!isField(target))return false;const value=choiceText(b,x);if(!value)return false;clearTimeout(timer);target.value=value;target.dataset.placeId=x?.placeId||'';active=target;selectedIndex=-1;commitGesture=true;const p=portal();p.classList.add('hide');closeTimer=setTimeout(()=>{p.innerHTML='';items=[];boundTarget=null;commitGesture=false},220);return true}
...
`;
if(!s.includes('carga();'))throw new Error('v163.15 startup anchor not found');s=s.replace('carga();',js+'\ncarga();');
const css=`
/* v163.15 safe Android autocomplete */
...
`;
if(!s.includes('</style>'))throw new Error('v163.15 css anchor not found');s=s.replace('</style>',css+'\n</style>');fs.writeFileSync('dist/index.html',s);console.log('Movvant v163.15: pointerdown commit autocomplete installed');