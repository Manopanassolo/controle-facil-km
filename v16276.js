...
(function(){
  ...
  function installShell(){
    ...
    n.querySelectorAll('[data-p]').forEach(b=>{if(b.dataset.mvAutoCollapse82)return;b.dataset.mvAutoCollapse82='1';b.addEventListener('click',()=>setTimeout(()=>{const target=b.dataset.p;if(valid(target)&&current!==target){const st=stackRead();if(current)st.push(current);stackWrite(st);setHistory(target,false);afterShow(target)}closeMenu()},0))});
