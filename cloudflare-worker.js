...
function googleReferer(request){
  const incoming=String(request.headers.get('referer')||'').trim();
  if(incoming)return incoming;
  try{return new URL(request.url).origin+'/'}catch{return 'https://movvant.panassolofilho.workers.dev/'}
}
...
async function places(request,env){
  ...
  const url=new URL(request.url),ref=googleReferer(request);
  ...
  const newHeaders=mask=>({'Content-Type':'application/json','Referer':ref,'X-Goog-Api-Key':key,'X-Goog-FieldMask':mask});
  ...
  if(!items.length){
    try{
      ...
      const r=await fetch(legacy,{headers:{Accept:'application/json',Referer:ref}});const j=await r.json().catch(()=>({}));
      if(r.ok&&j.status==='OK')merge((j.predictions||[]).map(p=>({placeId:p.place_id||'',text:p.description||'',mainText:p.structured_formatting?.main_text||p.description||'',secondaryText:p.structured_formatting?.secondary_text||'',types:p.types||[]})));else if(j.status!=='ZERO_RESULTS')apiError('places_legacy_autocomplete',r.status,j);
...
async function routes(request,env){
  ...
  try{
    ...
    const url=new URL(request.url),ref=googleReferer(request);let body={};if(isGet){for(const [k,v] of url.searchParams.entries())body[k]=v}else{body=await request.json().catch(()=>({}))}
    ...
    const r=await fetch('https://routes.googleapis.com/directions/v2:computeRoutes',{method:'POST',headers:{'Content-Type':'application/json','Referer':ref,'X-Goog-Api-Key':key,'X-Goog-FieldMask':fieldMask},body:JSON.stringify(payload)});const j=await r.json().catch(()=>({}));if(!r.ok)return json({configured:true,error:j?.error?.message||'routes_api_error',code:j?.error?.status||null,details:j},r.status);
...
export default {async fetch(request,env){const path=new URL(request.url).pathname;if(path==='/api/maps-health')return json({configured:!!env.GOOGLE_MAPS_API_KEY,places:'/api/places',routes:'/api/routes'});if(path==='/api/places')return places(request,env);if(path==='/api/routes')return routes(request,env);return env.ASSETS.fetch(request)}};
