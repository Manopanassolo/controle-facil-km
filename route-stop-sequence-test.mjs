...
const calls=[];
await page.route('**/api/routes',async route=>{const req=route.request();let body={};try{body=JSON.parse(req.postData()||'{}')}catch{}calls.push(body);const optimize=!!body.optimize;await route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({configured:true,items:[{distanceMeters:123400,duration:optimize?'7000s':'7600s',hasTolls:false,tollTotalBRL:0,optimizedIntermediateWaypointIndex:optimize?[1,0]:[],legs:[]}]})})});
const out={};
try{
  ...
  const input=page.locator('#preTripStopNameV127'),add=page.locator('#preTripStopAddV127');if(!await input.count()||!await add.count())throw new Error('stop editor missing');
  await input.scrollIntoViewIfNeeded();await input.fill('Chapecó, SC, Brasil');await add.click();await page.waitForTimeout(150);await input.fill('Concórdia, SC, Brasil');await add.click();await page.waitForTimeout(250);
  const before=await page.evaluate(()=>({rows:[...document.querySelectorAll('#preTripStopsListV127 .pre-stop-row-v127 b')].map(x=>x.textContent.trim()),source:globalThis.mvRouteStopsV16270?.()}));out.before=before;if(before.rows.length!==2||before.source?.length!==2)throw new Error('two visible stops not in unified source: '+JSON.stringify(before));
  const plan=page.locator('#mvPlanRouteV16271');await plan.waitFor();await plan.scrollIntoViewIfNeeded();await plan.click();await page.waitForSelector('[data-seq70]');await page.waitForTimeout(300);
  const result=await page.evaluate(()=>{const cards=[...document.querySelectorAll('[data-seq70]')].map(x=>x.innerText),f=document.getElementById('mvGoogleRouteFrameV16270');return {cards,frameSrc:f?.src||'',status:document.getElementById('routeMapStatusV133')?.textContent||'',options:(globalThis.mvRouteOptionsV16270||[]).map(o=>o.stops)}});out.result=result;out.calls=calls;
  const decoded=decodeURIComponent(result.frameSrc);out.pass=result.cards.length>=2&&result.cards.some(x=>x.includes('Chapecó'))&&result.cards.some(x=>x.includes('Concórdia'))&&decoded.includes('Chapecó')&&decoded.includes('Concórdia')&&result.status.includes('2 paradas')&&calls.some(x=>Array.isArray(x.stops)&&x.stops.length===2&&!x.optimize)&&calls.some(x=>Array.isArray(x.stops)&&x.stops.length===2&&x.optimize);
