...
await page.route('**/api/routes',async route=>{
  ...
  let items;
  if(body.origin==='Florianópolis, SC, Brasil'&&body.destination==='Galvão, SC, Brasil'&&Array.isArray(body.stops)&&body.stops.length===0){
    items=[
      {distanceMeters:110000,duration:'6500s',hasTolls:true,tollTotalBRL:11.40,optimizedIntermediateWaypointIndex:[],legs:[]},
      {distanceMeters:120000,duration:'7000s',hasTolls:false,tollTotalBRL:0,optimizedIntermediateWaypointIndex:[],legs:[]}
    ];
  }else{
    items=[{distanceMeters:123400,duration:body.optimize?'7000s':'7600s',hasTolls:true,tollTotalBRL:11.40,optimizedIntermediateWaypointIndex:body.optimize?[1,0]:[],legs:[]}];
  }
...
try{
  ...
  const input=page.locator('#preTripStopNameV127'),add=page.locator('#preTripStopAddV127');
  await input.scrollIntoViewIfNeeded();await input.fill('Chapecó, SC, Brasil');await add.click();await page.waitForTimeout(120);await input.fill('Concórdia, SC, Brasil');await add.click();await page.waitForTimeout(220);
  const before=await page.evaluate(()=>({rows:[...document.querySelectorAll('#preTripStopsListV127 .pre-stop-row-v127 b')].map(x=>x.textContent.trim()),source:globalThis.mvRouteStopsV16270?.()}));out.before=before;
  if(before.rows.length!==2||before.source?.length!==2)throw new Error('two stops missing from route source: '+JSON.stringify(before));
  const plan=page.locator('#mvPlanRouteV16272');await plan.waitFor();await plan.scrollIntoViewIfNeeded();await plan.click();await page.waitForSelector('[data-route72]');await page.waitForTimeout(350);
  const result=await page.evaluate(()=>{const cards=[...document.querySelectorAll('[data-route72]')].map(x=>x.innerText),f=document.getElementById('mvGoogleRouteFrameV16272');return {cards,frameSrc:f?.src||'',status:document.getElementById('routeMapStatusV133')?.textContent||'',options:(globalThis.mvRouteOptionsV16272||[]).map(o=>({distance:o.item.distanceMeters,duration:o.item.duration,toll:o.item.tollTotalBRL,stops:o.stops}))}});out.result=result;out.calls=calls;
  const decoded=decodeURIComponent(result.frameSrc),dists=result.options.map(o=>o.distance);
  out.pass=result.cards.length>=2&&result.cards.some(x=>x.includes('Opção de rota'))&&result.cards.every(x=>x.includes('Total ida + volta'))&&dists.every(x=>x>200000)&&result.cards.some(x=>x.includes('233,4 km'))&&decoded.includes('Chapecó')&&decoded.includes('Concórdia')&&result.status.includes('ida e volta')&&calls.some(x=>x.origin==='Florianópolis, SC, Brasil'&&x.destination==='Galvão, SC, Brasil'&&Array.isArray(x.stops)&&x.stops.length===0)&&calls.some(x=>Array.isArray(x.stops)&&x.stops.length===2&&x.optimize);
