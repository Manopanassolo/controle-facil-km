...
const names=[];
for(const m of js.matchAll(/\bfunction\s+([A-Za-z_$][\w$]*)\s*\(/g)) names.push(m[1]);
const dup=[...new Set(names.filter((n,i)=>names.indexOf(n)!==i))];
if(dup.length) throw new Error('Funções duplicadas no módulo: '+dup.join(', '));
const markup=html.split('<script type="module">')[0];
