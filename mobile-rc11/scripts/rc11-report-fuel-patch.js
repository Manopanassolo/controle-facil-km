...
const newFuel=" const fuelSummary=summarizeFuelEconomics({km:kmTotal,fuelings,remoteExpenses:expenses,vehicleRefs:vehicles,from:start,to:end});\n const fuelMode=fuelSummary.mode==='approved-real'||fuelSummary.mode==='remote-real'?'real':'estimated',fuelCost=fuelSummary.cost,realLiters=fuelSummary.mode==='approved-real'||fuelSummary.mode==='remote-real'?fuelSummary.liters:0,estimatedLiters=fuelSummary.mode==='estimated'?fuelSummary.liters:0,refAvg=fuelSummary.mode==='unavailable'?0:1;";
if(!text.includes(newFuel)){
 if(text.includes(oldFuel))text=text.replace(oldFuel,newFuel);else console.log('Report fuel calculation target changed');
}
