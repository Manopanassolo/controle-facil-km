...
const KEY = 'movvant.rc11.enterprise.settings';

export type EnterpriseSettings = {
  visit: {
    geofenceMeters: number;
    proximityCheckSeconds: number;
    scheduleWindowMinutes: number;
    suggestCheckin: boolean;
    requireGpsCheckin: boolean;
    requireGpsCheckout: boolean;
    minimumVisitMinutes: number;
  };
  tracking: {
    gpsDistanceIntervalMeters: number;
    gpsTimeIntervalSeconds: number;
    minimumTrackStepMeters: number;
    maximumTrackStepMeters: number;
    keepFullLocalTrack: boolean;
  };
  sync: {
    autoSync: boolean;
    retrySeconds: number;
    keepOfflineData: boolean;
  };
  agenda: {
    reminderMinutes: number;
    allowLocalAppointments: boolean;
  };
  km: {
    requireVehicle: boolean;
    requirePhoto: boolean;
    allowManualKm: boolean;
  };
  notifications: {
    operational: boolean;
    agenda: boolean;
...
export const DEFAULT_ENTERPRISE_SETTINGS: EnterpriseSettings = {
  visit: {
    geofenceMeters: 200,
    proximityCheckSeconds: 60,
    scheduleWindowMinutes: 120,
    suggestCheckin: true,
    requireGpsCheckin: true,
    requireGpsCheckout: false,
    minimumVisitMinutes: 0,
  },
  tracking: {
    gpsDistanceIntervalMeters: 5,
    gpsTimeIntervalSeconds: 2,
    minimumTrackStepMeters: 2,
    maximumTrackStepMeters: 300,
    keepFullLocalTrack: true,
  },
...
export function normalizeEnterpriseSettings(value: Partial<EnterpriseSettings> | null | undefined): EnterpriseSettings {
  ...
  const merged: EnterpriseSettings = {
    visit: { ...DEFAULT_ENTERPRISE_SETTINGS.visit, ...(v.visit || {}) },
    tracking: { ...DEFAULT_ENTERPRISE_SETTINGS.tracking, ...(v.tracking || {}) },
    sync: { ...DEFAULT_ENTERPRISE_SETTINGS.sync, ...(v.sync || {}) },
    agenda: { ...DEFAULT_ENTERPRISE_SETTINGS.agenda, ...(v.agenda || {}) },
    km: { ...DEFAULT_ENTERPRISE_SETTINGS.km, ...(v.km || {}) },
    notifications: { ...DEFAULT_ENTERPRISE_SETTINGS.notifications, ...(v.notifications || {}) },
  };
...
export async function readEnterpriseSettings(): Promise<EnterpriseSettings> {
  const raw = await AsyncStorage.getItem(KEY);
  if (!raw) return DEFAULT_ENTERPRISE_SETTINGS;
  try { return normalizeEnterpriseSettings(JSON.parse(raw)); } catch { return DEFAULT_ENTERPRISE_SETTINGS; }
}

export async function saveEnterpriseSettings(settings: EnterpriseSettings) {
