...
export type VehicleRow = { id: string; company_id: string; branch_id?: string | null; plate: string; make?: string | null; model?: string | null; current_odometer_km?: number | string | null; fuel_type?: string | null; avg_km_per_liter?: number | string | null; reference_fuel_price?: number | string | null; tank_capacity_liters?: number | string | null; active: boolean };
...
export async function loadBootstrap(session: Session): Promise<BootstrapData> {
  ...
  const [branches, assignments, vehicles, customers, visits, notifications, performanceRows, routes] = await Promise.all([
    ...
    safe(() => restSelect<VehicleRow>(session, 'vehicles', `select=id,company_id,branch_id,plate,make,model,current_odometer_km,fuel_type,avg_km_per_liter,reference_fuel_price,tank_capacity_liters,active&company_id=eq.${enc(companyId)}&active=eq.true&order=plate.asc`), [], errors, 'veículos'),
