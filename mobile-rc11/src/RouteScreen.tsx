...
type Props = { onSaved?: () => void; companyId?: string | null; customers?: CustomerRow[]; plannedCustomerIds?: string[]; plannedTripId?: string | null; plannedDistanceMeters?: number; plannedDurationSeconds?: number; plannedTollBRL?: number };
...
export default function RouteScreen({ onSaved, companyId, customers = [], plannedCustomerIds = [], plannedTripId = null, plannedDistanceMeters = 0, plannedDurationSeconds = 0, plannedTollBRL = 0 }: Props) {
  ...
  const finish = async () => {
    try {
      ...
      const distanceVarianceMeters = plannedDistanceMeters > 0 ? Math.round(distance - plannedDistanceMeters) : undefined;
