...
(function(){
  ...
  async function syncOne(id,quiet=false){if(!id||lastSynced===id)return false;const r=await api('/api/google-calendar/sync',{method:'POST',body:JSON.stringify({schedule_id:id})});if(r.ok&&r.data?.synced){lastSynced=id;await sb.from('km_scheduled_trips').select('id').eq('id',id).maybeSingle().catch?.(()=>{});if(!quiet&&typeof msg==='function')msg('Viagem sincronizada com o Google Agenda.');return true}if(!quiet&&r.status===409&&typeof msg==='function')msg('Viagem salva. Conecte o Google Agenda para sincronizar.');return false}
