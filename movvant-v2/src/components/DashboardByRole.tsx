...
export function DashboardByRole() {
  ...
  return (
    <>
      ...
      {activityCount ? <section className="panel session-dashboard-strip" aria-label="Indicadores da homologação local"><div><span className="eyebrow">Atividade desta sessão</span><strong>{activityCount} evento(s) consolidado(s)</strong><small>Modo Campo, Rotas e Custos alimentam este resumo. Recarregar a aplicação limpa os dados.</small></div><div className="session-dashboard-metrics"><span><b>{totalKm.toLocaleString('pt-BR')} km</b><small>KM da sessão</small></span><span><b>{sessionRoutes}</b><small>deslocamentos concluídos</small></span><span><b>{journeys.length}</b><small>visitas/jornadas</small></span><span><b>R$ {totalExpenses.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</b><small>despesas da sessão</small></span><span><b>{expenses.length}</b><small>lançamentos independentes</small></span></div><div className="session-dashboard-actions"><Link href="/historico" className="secondary-button">Ver histórico</Link><Link href="/relatorios" className="secondary-button">Ver relatório</Link></div></section> : null}
