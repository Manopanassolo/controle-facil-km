...
export function PendingSessionModule(){
  const {maintenanceAlerts,maintenancePendingCount}=useSessionActivity();
  ...
  return <section className="panel">
    ...
    <div className="pending-list">
      {alerts.map((alert)=><article className="pending-row session-row" key={alert.vehicleId}><div className={`priority-mark ${alert.state==='vencida'?'critical':'high'}`}/><div className="pending-copy"><div className="pending-title-line"><strong>{alert.state==='vencida'?'Revisão vencida':'Revisão próxima'} · {alert.vehicle}</strong><span className="tag">{alert.state==='vencida'?'Crítica':'Alta'}</span></div><span>{alert.plate} · {alert.dueKm.toLocaleString('pt-BR')} km ou {formatDate(alert.dueDate)}</span><span>{alert.state==='vencida'?'Veículo indisponível para novas rotas':`${Math.max(0,alert.remainingKm).toLocaleString('pt-BR')} km restantes · ${Math.max(0,alert.remainingDays)} dia(s)`}</span></div><Link href="/veiculos" className="secondary-button">Ver veículo</Link></article>)}
    ...
    <div className="soft-box"><strong>Regra da V2</strong><span>Manutenção vencida bloqueia novas rotas; manutenção próxima gera alerta sem bloquear. Notificações meramente informativas continuam fora desta central.</span></div>
  </section>;
}
