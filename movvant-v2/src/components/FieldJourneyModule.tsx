...
export function FieldJourneyModule() {
  ...
  const { journeys, addJourney, vehicleOptions } = useSessionActivity();
  const index = step === 'idle' ? 0 : step === 'route' ? 1 : step === 'arrived' ? 2 : step === 'visited' ? 3 : step === 'expense' ? 4 : 5;

  function start(values: PrototypeFormValues) { setJourney({ vehicle:text(values,'veiculo'), kmStart:Number(text(values,'kmInicial'))||0 }); setStep('route'); }
