...
export function FieldJourneyModule() {
  ...
  function start(values: PrototypeFormValues) {
    setJourney({ vehicle: text(values, 'veiculo'), kmStart: Number(text(values, 'kmInicial')) || 0 });
    setStep('route');
  }

  function arrive(values: PrototypeFormValues) {
    setJourney((current) => ({ ...current, client: text(values, 'cliente'), arrivalNote: text(values, 'observacao') }));
    setStep('arrived');
  }

  function visit(values: PrototypeFormValues) {
    setJourney((current) => ({ ...current, visitResult: `${text(values, 'resultado')} · ${text(values, 'proximoPasso')}` }));
    setStep('visited');
  }
  ...
  return (
    <section className="field-mode-layout">
      <article className="panel field-mode-hero">
        <span className="eyebrow">Operação guiada · celular</span>
        <h2>{step === 'done' ? 'Jornada concluída' : 'Seu dia em campo'}</h2>
        <p>Homologue a sequência completa de trabalho sem sair desta tela e sem gravar dados no backend.</p>
        <div className="field-status"><span className={`status-dot ${step === 'done' ? '' : 'preview'}`} /><strong>{step === 'idle' ? 'Pronto para iniciar' : step === 'done' ? 'Fluxo concluído' : `Etapa ${Math.min(index + 1, 5)} de 5`}</strong><span>Dados somente nesta sessão</span></div>
      </article>

      <div className="journey-progress" aria-label="Progresso da jornada">
        {steps.map(([number, title], itemIndex) => <div className={`journey-progress-step ${itemIndex < index ? 'complete' : itemIndex === index && step !== 'done' ? 'current' : ''}`} key={number}><span>{number}</span><strong>{title}</strong></div>)}
      </div>

      <article className="panel journey-action-panel">
        {step === 'idle' ? <PrototypeFormDialog trigger="01 · Iniciar rota" title="Iniciar rota" description="Registre veículo e KM de saída." onValidate={start} fields={[{name:'veiculo',label:'Veículo',type:'select',required:true,options:['SUV Comercial','Hatch Vendas','Utilitário']},{name:'kmInicial',label:'KM inicial',type:'number',required:true,placeholder:'12480'},{name:'destino',label:'Primeiro destino',required:true,placeholder:'Cliente ou cidade'}]} /> : null}
        ...
        {step === 'expense' ? <PrototypeFormDialog trigger="05 · Encerrar rota" title="Encerrar rota" description="Finalize o dia com o KM final." onValidate={finish} fields={[{name:'kmFinal',label:'KM final',type:'number',required:true,placeholder:String((journey.kmStart || 0) + 30)},{name:'resumo',label:'Resumo final',type:'textarea',placeholder:'Ocorrências ou observações do retorno'}]} /> : null}
