...
export function HistorySessionModule(){
 ...
 const{incidents,clearIncidents}=useIncidentSession();
 const sessionItems=[
  ...
  ...incidents.map((item)=>({id:item.id,createdAt:item.resolvedAt||item.createdAt,title:`${item.status==='Concluído'?'Sinistro concluído':'Sinistro registrado'} · ${item.vehicle}`,detail:`${item.driver} · ${item.location} · ${item.severity} · ${item.description}${item.resolution?` · ${item.resolution}`:''}`,type:'Sinistro'}))
 ].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
 const totalActivity=activityCount+incidents.length;
 function clearSession(){clearAllActivity();clearIncidents();}
 return <section className="panel"><div className="panel-title-row"><div><h2>Linha do tempo operacional</h2>{totalActivity?<span className="session-chip">{totalActivity} registro(s) desta sessão</span>:null}</div><div className="row-actions"><PrototypeActionButton className="secondary-button" title="Filtrar histórico" description="Abrirá período, usuário, veículo, cidade e tipo de atividade para refinar a linha do tempo.">Filtrar período</PrototypeActionButton>{totalActivity?<button className="secondary-button" type="button" onClick={clearSession}>Limpar sessão</button>:null}</div></div><div className="timeline-list">{sessionItems.map((item)=>{const time=new Date(item.createdAt).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});return <div className="timeline-entry session-row" key={item.id}><span className="timeline-date">Hoje · {time}</span><i className="timeline-dot"/><div className="timeline-content"><strong>{item.title}</strong><span>{item.detail}</span><div className="history-session-tags"><em className="session-chip">{item.type}</em><em className="session-chip">Somente nesta sessão</em></div></div></div>;})}{!sessionItems.length?<div className="soft-box"><strong>Histórico sem registros</strong><span>Nenhum histórico fictício é exibido. Novos eventos reais aparecerão aqui conforme a operação for utilizada.</span></div>:null}</div></section>;
}
