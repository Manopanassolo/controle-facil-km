...
type ReportType='Todos'|'Jornada'|'Rota'|'Despesa'|'Agenda'|'Manutenção'|'Documento'|'Sinistro';
...
export function ReportsSessionModule(){
 const{journeys,routes,expenses,appointments,maintenanceRecords,maintenanceAlerts,documents,documentAlerts,totalKm,totalExpenses}=useSessionActivity();
 ...
 const rows=useMemo<Row[]>(()=>[
  ...
  ...documents.map(x=>{const alert=documentAlerts.find(a=>a.documentId===x.id);return{id:x.id,createdAt:x.createdAt,type:'Documento' as const,vehicle:x.scope==='veiculo'?x.subject:'—',description:`${x.kind} · ${x.subject} · validade ${x.expiryDate}`,km:0,amount:0,status:alert?.state==='vencido'?'Vencido':alert?.state==='proximo'?'Próximo do vencimento':'Válido'};}),
 ...
 ].sort((a,b)=>b.createdAt.localeCompare(a.createdAt)),[journeys,routes,expenses,appointments,maintenanceRecords,documents,documentAlerts,incidents]);
