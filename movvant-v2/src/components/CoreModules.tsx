...
export function CostsModule() {
  ...
  return <><section className="dashboard-grid"><article className="metric-card"><span className="metric-label">Total no mês</span><strong className="metric-value">R$ 1.024</strong><div className="metric-note">despesas registradas</div></article><article className="metric-card"><span className="metric-label">Combustível</span><strong className="metric-value">R$ 782</strong><div className="metric-note">76% do total</div></article><article className="metric-card"><span className="metric-label">Custo por KM</span><strong className="metric-value">R$ 0,82</strong><div className="metric-note">média mensal</div></article><article className="metric-card"><span className="metric-label">Pendentes</span><strong className="metric-value">2</strong><div className="metric-note">lançamentos para revisar</div></article></section><section className="panel data-panel"><div className="panel-title-row"><h2>Lançamentos recentes</h2><PrototypeActionButton title="Nova despesa" description="O fluxo incluirá categoria, valor, data, veículo, KM, local, observação e comprovante, com validação antes de salvar.">+ Nova despesa</PrototypeActionButton></div><div className="data-table">{rows.map(([date,type,place,value]) => <div className="data-row" key={`${date}-${type}`}><span>{date}</span><strong>{type}</strong><span>{place}</span><b>{value}</b></div>)}</div></section></>;
}
