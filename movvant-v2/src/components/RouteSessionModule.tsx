...
export function RouteSessionModule() {
  ...
  const { routes, addRoute, vehicleOptions } = useSessionActivity();

  function startRoute(values: PrototypeFormValues) {
    setActive({ origin: text(values, 'origem'), destination: text(values, 'destino'), vehicle: text(values, 'veiculo'), purpose: text(values, 'finalidade'), kmStart: Number(text(values, 'kmInicial')) || 0, status: 'Em andamento' });
  }
  ...
  return <section className="route-session-layout">
    <article className="panel route-session-control">
      ...
      <div className="session-banner compact"><strong>{vehicleOptions.length} veículo(s) operacional(is)</strong><span>Fonte única da frota V2 · manutenção fica indisponível.</span></div>
      ...
      <div className="route-session-actions">{!active ? <PrototypeFormDialog trigger="Iniciar rota" title="Iniciar rota" description="Selecione um veículo da frota compartilhada. Ao encerrar, a rota passa para Dashboard, Histórico e Relatórios." onValidate={startRoute} fields={[
