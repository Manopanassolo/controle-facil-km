...
export function RouteSessionModule() {
  ...
  function startRoute(values: PrototypeFormValues) {
    setActive({
      ...
      kmStart: Number(text(values, 'kmInicial')) || 0,
      status: 'Em andamento'
    });
  }
  ...
  return (
    <section className="route-session-layout">
      <article className="panel route-session-control">
        <div className="panel-title-row">
          <div><span className="eyebrow">Homologação operacional</span><h2>{active ? 'Rota em andamento' : 'Controle de deslocamento'}</h2></div>
          <span className={`tag ${active ? 'warning' : 'success'}`}>{active ? 'Em andamento' : 'Pronto'}</span>
        </div>

        {active ? (
          <div className="route-live-card">
            <div><span>Origem</span><strong>{active.origin}</strong></div>
            <div><span>Destino</span><strong>{active.destination}</strong></div>
            <div><span>Veículo</span><strong>{active.vehicle}</strong></div>
            <div><span>KM inicial</span><strong>{active.kmStart.toLocaleString('pt-BR')} km</strong></div>
            <div className="route-live-wide"><span>Finalidade</span><strong>{active.purpose}</strong></div>
          </div>
        ) : (
          <div className="soft-box"><strong>Nenhuma rota ativa</strong><span>Inicie um deslocamento para homologar KM inicial, execução e encerramento.</span></div>
        )}
