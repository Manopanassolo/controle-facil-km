...
def create_agenda_item(page):
    ...
    dialog=page.get_by_role('dialog')
    dialog.locator('input[name="titulo"]').fill('Visita E2E')
    dialog.locator('input[name="cliente"]').fill('Cliente Automação')
    dialog.locator('input[name="data"]').fill('2026-09-02')
    dialog.locator('input[name="horario"]').fill('09:30')
    dialog.locator('input[name="endereco"]').fill('Rua Teste, 100, Itajaí')
    dialog.locator('input[name="responsavel"]').fill('Rafael Silva')
