...
const js=`
// v163.06: single desktop brand authority. Autocomplete authority lives in v16292.js.
(function(){
  ...
  function cleanBrands(){
    ...
    document.body.classList.add('mv-brand106');
  }
  ...
  globalThis.mvBrandV16306={sync:cleanBrands};
})();
`;
if(!s.includes('carga();'))throw new Error('v163.06 branding anchor');s=s.replace('carga();',js+'\ncarga();');
const css=`
/* v163.06 single desktop brand */
@media(min-width:900px){body.mv-brand106 #p-inicio{padding-top:12px!important}}
`;
if(!s.includes('</style>'))throw new Error('v163.06 css anchor');s=s.replace('</style>',css+'\n</style>');
fs.writeFileSync('dist/index.html',s);console.log('Movvant v163.06: single desktop brand authority installed');
