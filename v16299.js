...
const js=`
// v163.04: deterministic two-tap target binding + single desktop brand authority.
(function(){
  ...
  // Complete only the SECOND tap. Use the visible suggestion text and the most recently typed field,
  // so a stop can never inherit a stale origin result from the older autocomplete closure.
  ...
  function cleanBrands(){
    ...
    if(nav){
      // Sidebar currently renders two Movvant identities. Keep the larger canonical one and remove the tiny legacy header.
    ...
    // Remove the centered duplicate brand above Dashboard, preserving sidebar brand.
    ...
    document.body.classList.add('mv-brand104');
  }
  ...
  globalThis.mvV16304={remember,cleanBrands};
})();
`;
if(!s.includes('carga();'))throw new Error('v163.04 startup anchor');s=s.replace('carga();',js+'\ncarga();');
const css=`
/* v163.04 single brand + deterministic autocomplete */
@media(min-width:900px){body.mv-brand104 #p-inicio{padding-top:12px!important}}
`;
if(!s.includes('</style>'))throw new Error('v163.04 css anchor');s=s.replace('</style>',css+'\n</style>');
fs.writeFileSync('dist/index.html',s);console.log('Movvant v163.04: deterministic stop autocomplete and single desktop brand authority installed');
