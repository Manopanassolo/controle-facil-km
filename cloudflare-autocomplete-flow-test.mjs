...
try{
  ...
  result.destination=await twoTap('destino','Aeroporto Congonhas','destination');assertTwoTap('Destino',result.destination,'Aeroporto de Congonhas');await page.locator('#destino').focus();result.destinationUsable=await page.locator('#destino').evaluate(el=>!el.disabled&&!el.readOnly&&document.activeElement===el);if(!result.destinationUsable)throw new Error('Destino not usable after stop');
