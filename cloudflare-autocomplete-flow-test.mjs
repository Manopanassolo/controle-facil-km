...
const watchdog=setTimeout(()=>{result.pass=false;result.error='watchdog timeout at '+stage;try{fs.writeFileSync('/tmp/movvant-autocomplete-flow.json',JSON.stringify(result,null,2))}catch{}console.error('FLOW_WATCHDOG',stage);process.exit(2)},32000);
...
async function ensurePlannerOpen(){
  await page.evaluate(()=>{const fold=document.getElementById('mvRouteFold93');if(fold&&!fold.open)fold.open=true;const route=document.getElementById('directRouteStackV127');if(route){route.classList.remove('hide');route.style.setProperty('display','block','important');route.style.setProperty('visibility','visible','important')}});
  await page.waitForTimeout(80);
}
async function expose(){await page.evaluate(()=>{const auth=document.getElementById('auth'),app=document.getElementById('app'),trip=document.getElementById('p-viagem'),form=document.getElementById('novaViagem'),route=document.getElementById('directRouteStackV127'),fold=document.getElementById('mvRouteFold93');auth?.classList.add('hide');if(app){app.classList.remove('hide');app.style.setProperty('display','block','important')}document.body.classList.add('mv-trip-active-v16247');document.body.dataset.mvRoute='viagem';if(fold)fold.open=true;for(const root of [trip,form,route]){let n=root;while(n&&n!==document.body){n.classList?.remove('hide');n.style?.setProperty('display','block','important');n.style?.setProperty('visibility','visible','important');n=n.parentElement}}route?.scrollIntoView({block:'center'})});await page.waitForTimeout(160)}
async function twoTap(fieldId,text,label){
  await ensurePlannerOpen();mark(label+'-fill');const input=page.locator('#'+fieldId);await input.fill('');await input.focus();await input.pressSequentially(text,{delay:15});
...
try{
  ...
  await ensurePlannerOpen();mark('toggle-visible');const toggle=page.locator('#mvStopToggleV16262');await toggle.waitFor({state:'visible'});await toggle.scrollIntoViewIfNeeded();mark('toggle-click');await toggle.click({timeout:3000});mark('toggle-clicked');
  ...
  await ensurePlannerOpen();mark('stop-add');await page.locator('#preTripStopAddV127').click({timeout:3000});await page.waitForTimeout(120);result.stopAdded=await page.locator('#preTripStopsListV127').innerText();if(!result.stopAdded.includes('Praça da Sé'))throw new Error('Stop was not added');mark('stop-added');
